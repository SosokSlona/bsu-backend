...
# СЛОВА, КОТОРЫЕ НЕ МОГУТ БЫТЬ ГРУППОЙ (Запрещенный список)
FORBIDDEN_GROUP_WORDS = [
    'понедельник', 'вторник', 'среда', 'четверг', 'пятница', 'суббота', 
    'день', 'дни', 'время', 'часы', 'гревтеч', 'киньледеноп'
]
...
def parse_schedule_pdf(pdf_bytes: bytes, course: int) -> ParsedScheduleResponse:
    ...
    for pg_num, img in enumerate(images):
        ...
        # Если время не нашли, считаем 2-ю колонку временем
        if time_col is None and len(rows[0]) > 2:
            time_col = 1
        
        # Если групп не нашли, берем всё правее времени, НО ПРОВЕРЯЕМ ФИЛЬТРОМ
        if not group_cols: 
             t_idx = time_col if time_col else 1
             for i in range(t_idx + 1, len(rows[0])):
                 # Дополнительная проверка: не является ли колонка "Днем недели"?
                 if not _is_forbidden_column(rows, i, roi_img):
                     group_cols.append(i)
    ...
    for g_name, days in schedule_by_group.items():
        # Если в группе нет ни одного урока - это фантом, удаляем
...
def _analyze_column_roles(header_row, img):
    ...
    for i, rect in enumerate(header_row):
        ...
        # Жесткая проверка на слова-паразиты
        if any(f in txt for f in FORBIDDEN_GROUP_WORDS):
            # Если нашли "Понедельник" - это точно ДЕНЬ, а не группа
            if any(d in txt for d in ['понедельник', 'вторник']):
                roles['day'] = i
            continue 

        if any(t in txt for t in ['время', 'часы', '8.30']):
            roles['time'] = i
...
def _is_forbidden_column(rows, col_idx, img):
    """Проверяет первые 5 строк колонки на наличие дней недели"""
    for r in rows[:5]:
